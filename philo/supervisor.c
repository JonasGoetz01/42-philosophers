...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/21 21:11:26 by jgotz             #+#    #+#             */
/*   Updated: 2024/02/21 21:11:26 by jgotz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&(tbl->remain_mutex));
	if (tbl->philos_remaining < 1)
	{
		pthread_mutex_unlock(&(tbl->remain_mutex));
		return (1);
	}
	pthread_mutex_unlock(&(tbl->remain_mutex));
	return (0);
...
void	*supervisor_check(t_table *tbl)
{
	...
	while (i < tbl->philos_amt)
	{
		if (all_ate_check(tbl))
			return (stop_sim(tbl, 0, 0), NULL);
		time = get_elapsed(tbl);
		if (is_dead(time, &(tbl->philos[i])))
		{
			return (stop_sim(tbl, time, i + 1), NULL);
		}
		i++;
	}
	return ((void *)tbl);
