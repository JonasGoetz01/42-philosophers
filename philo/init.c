...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/25 14:18:15 by jgotz             #+#    #+#             */
/*   Updated: 2024/01/25 14:38:46 by jgotz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	alloc(t_data *data)
{
	...
	if (!data->tid)
		return (error("error allocating thread", data));
	data->forks = malloc(sizeof(pthread_mutex_t) * data->philo_num);
	if (!data->forks)
		return (error("error allocating philosophers", data));
	data->philos = malloc(sizeof(t_philo) * data->philo_num);
	if (!data->philos)
		return (error("error allocating fork", data));
	return (0);
}
